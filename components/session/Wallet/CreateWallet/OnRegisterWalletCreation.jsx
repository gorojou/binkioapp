...
export default function OnRegisterWalletCreation() {
  ...
  const uploadWallets = async () => {
    try {
      setLoading(true);
      await uploadWallet(wallets.btc, wallets.btc.nombre, "btc");
      await uploadWallet(wallets.eth, wallets.eth.nombre, "eth");
      await updateProfile();
    } catch (err) {
      setLoading(false);
  ...
  return (
    <>
      <>
        <View style={[styles.formulario, { flex: 1 }]}>
          ...
          >
            ...
            <MainButton
              style={{ marginTop: 20 }}
              width={0.9}
              callback={() => uploadWallets()}
              blocked={!wallets.btc || !wallets.eth}
            >
              Guardar
            </MainButton>
          </View>
        ...
        {generateType && (
          <>
            <Popup setShow={setGenerateType} closingValue={null}>
              <WalletCreation
                confirmWallet={true}
